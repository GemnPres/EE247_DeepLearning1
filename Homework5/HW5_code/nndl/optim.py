...
def sgd_momentum(w, dw, config=None):
    ...
    v = alpha*v - lr*dw  # first step only has grad since v=0
    next_w = w + v

    # We save the updated velocity in config dict and now we start to accumulate momentum
...
def sgd_nesterov_momentum(w, dw, config=None):
    ...
    lr = config["learning_rate"]
    v_old = v
    v_new = alpha*v_old - lr*dw  # first step only has grad since v=0
    next_w = w + v_new + alpha*(v_new - v_old)
    v = v_new
...
def rmsprop(w, dw, config=None):
    ...
    lr = config['learning_rate']
    nu = config['epsilon']
    a = beta*a + (1-beta)*(np.multiply(dw, dw))
    next_w = w - np.multiply(lr/(np.sqrt(a)+nu), dw)
    config['a'] = a # remember to update 2nd moment coeff a 
...
def adam(w, dw, config=None):
    ...
    v_new = beta1*v_old + (1-beta1)*dw # First moment like momentum
    a_new = beta2*a_old + (1-beta2)*np.multiply(dw, dw) # Second moment like adagrad
    
    # Bias term such that first and second moment have similar magnitude
    # Without bias, when a is small, we would take a huge update step which derogates our result
    t_new = t_old+1
    v_tilde = 1/(1-pow(beta1,t_new))*v_new
    a_tilde = 1/(1-pow(beta2,t_new))*a_new

    next_w = w - np.multiply(lr/(np.sqrt(a_tilde) + nu), v_tilde)
