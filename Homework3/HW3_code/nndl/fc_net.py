...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    """ FORWARD
    layer1, layer1_cache = affine_forward(X, W1, b1)
    cache_ls2.append(layer1_cache)
    
    ReLU_act, ReLU_act_cache = relu_forward(layer1)
    cache_ls2.append(ReLU_act_cache)
    
    layer2, layer2_cache = affine_forward(ReLU_act, W2, b2)
    cache_ls2.append(layer2_cache)

    ReLU_act2, ReLU_act_cache2 = relu_forward(layer2)
    cache_ls2.append(ReLU_act2)

    scores, layer3_cache = affine_forward(ReLU_act2, W3, b3)
    ...
    """BACKWARD
    grad_ReLU2, grad_W3, grad_b3 = affine_backward(grad_softmax, layer_cache[4])  #i==5
    grad_layer2 = relu_backward(grad_ReLU2, layer_cache[3])  #i==4
    grad_ReLU1, grad_W2, grad_b2 = affine_backward(grad_layer2, layer_cache[2])   #i==3
    grad_layer1 = relu_backward(grad_ReLU1, layer_cache[1])  #i==2
    _, grad_W1, grad_b1 = affine_backward(grad_layer1, layer_cache[0])   #i==1
    """
    
    # len(cache_ls) is guaranteed to be an odd num due to sandwitching (no activation @ output layer)
